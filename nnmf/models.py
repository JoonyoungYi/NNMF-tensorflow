...
class NNMF(_NNMFBase):
    ...
    def _init_ops(self):
        ...
        f_train_step = self.optimizer.minimize(
            self.loss, var_list=[self.mlp_weights[key] for key in self.mlp_weights])
...
class SVINNMF(_NNMFBase):
    ...
    def _init_vars(self):
        # Latents
        self.U_mu = tf.Variable(
            tf.truncated_normal([self.num_users, self.D], **
                                self.latent_normal_init_params))
        self.U_log_var = tf.Variable(
            tf.random_uniform(
                [self.num_users, self.D], minval=0.0, maxval=0.5))

        self.Uprime_mu = tf.Variable(
            tf.truncated_normal([self.num_users, self.Dprime], **
                                self.latent_normal_init_params))
        self.Uprime_log_var = tf.Variable(
            tf.random_uniform(
                [self.num_users, self.Dprime], minval=0.0, maxval=0.5))

        self.V_mu = tf.Variable(
            tf.truncated_normal([self.num_items, self.D], **
                                self.latent_normal_init_params))
        self.V_log_var = tf.Variable(
            tf.random_uniform(
                [self.num_items, self.D], minval=0.0, maxval=0.5))

        self.Vprime_mu = tf.Variable(
            tf.truncated_normal([self.num_items, self.Dprime], **
                                self.latent_normal_init_params))
        self.Vprime_log_var = tf.Variable(
            tf.random_uniform(
        ...
        Uprime_log_var_lu = tf.nn.embedding_lookup(self.Uprime_log_var,
                                                   self.user_index)
        ...
        Vprime_log_var_lu = tf.nn.embedding_lookup(self.Vprime_log_var,
                                                   self.item_index)

        # Posterior (q) - note this handles reparameterization for us
        q_U = tf.contrib.distributions.MultivariateNormalDiag(
            mu=U_mu_lu, diag_stdev=tf.sqrt(tf.exp(U_log_var_lu)))
        q_Uprime = tf.contrib.distributions.MultivariateNormalDiag(
            mu=Uprime_mu_lu, diag_stdev=tf.sqrt(tf.exp(Uprime_log_var_lu)))
        q_V = tf.contrib.distributions.MultivariateNormalDiag(
            mu=V_mu_lu, diag_stdev=tf.sqrt(tf.exp(V_log_var_lu)))
        q_Vprime = tf.contrib.distributions.MultivariateNormalDiag(
            mu=Vprime_mu_lu, diag_stdev=tf.sqrt(tf.exp(Vprime_log_var_lu)))
        ...
        f_input_layer = tf.concat(
            concat_dim=1,
            values=[self.U, self.V,
                    tf.multiply(self.Uprime, self.Vprime)])

        self.r_mu, self.mlp_weights = build_mlp(
            f_input_layer, hidden_units_per_layer=self.hidden_units_per_layer)
        self.r = tf.squeeze(self.r_mu, squeeze_dims=[1])

        # For KL annealing
        self.kl_weight = tf.placeholder(
            tf.float32) if self.anneal_kl else tf.constant(
    ...
    def _init_ops(self):
        ...
        KL_Uprime = KL(
            self.Uprime_mu,
            self.Uprime_log_var,
            prior_var=self.Uprime_prior_var)
        KL_V = KL(self.V_mu, self.V_log_var, prior_var=self.V_prior_var)
        KL_Vprime = KL(
            self.Vprime_mu,
            self.Vprime_log_var,
            prior_var=self.Vprime_prior_var)
    ...
    def train_iteration(self, data):
        ...
        super(SVINNMF, self)._train_iteration(
            data, additional_feed=additional_feed)
    ...
    def eval_loss(self, data):
        ...
        feed_dict = {
            ...
            self.item_index: item_ids,
            self.r_target: ratings,
            self.kl_weight: get_kl_weight(
                self._iters, on_iter=self.kl_full_iter)
        }
        return self.sess.run(self.loss, feed_dict=feed_dict)
